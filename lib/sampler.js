'use strict'

const EventEmitter = require('events')
const EventLoopDelayMetric = require('./eventLoopDelay')
const CpuMetric = require('./cpu')
const GCMetric = require('./gc')
const config = require('./config')
const {
  kSamplerOptions,
  kSamplerTimer,
  kStarted,
  kLastSampleTime,
  kEventLoopDelay,
  kCpu,
  kGC,
  kActiveHandles,
  kMemory,
  kEmitSample,
  kSample,
  kReset
} = require('./symbols')

function hrtime2ns (time) {
  return time[0] * 1e9 + time[1]
}

class Sampler extends EventEmitter {
  constructor (options) {
    super()
    this[kSamplerOptions] = config(options)
    this[kLastSampleTime] = process.hrtime()

    if (this[kSamplerOptions].collect.eventLoopDelay) {
      this[kEventLoopDelay] = new EventLoopDelayMetric(options.eventLoopOptions)
    }

    if (this[kSamplerOptions].collect.cpu) {
      this[kCpu] = new CpuMetric()
    }

    if (this[kSamplerOptions].collect.gc) {
      this[kGC] = new GCMetric()
    }

    this[kSample]()
    if (this[kSamplerOptions].autoStart) {
      this.start()
    }
  }

  start () {
    if (this[kStarted]) return
    this[kSamplerTimer] = setInterval(this[kEmitSample].bind(this), this[kSamplerOptions].sampleInterval)
    if (this[kSamplerOptions].unref) {
      this[kSamplerTimer].unref()
    }
    this[kStarted] = true
  }

  stop () {
    clearInterval(this[kSamplerTimer])
    this[kStarted] = false
  }

  get cpu () {
    return this[kCpu]
  }

  get eventLoopDelay () {
    return this[kEventLoopDelay]
  }

  get gc () {
    return this[kGC]
  }

  get activeHandles () {
    return this[kActiveHandles]
  }

  get memory () {
    return this[kMemory]
  }

  [kEmitSample] () {
    this[kSample]()
    this.emit('sample')
    this[kReset]()
  }

  [kSample] () {
    const nextSampleTime = process.hrtime()
    const elapsedNs = hrtime2ns(nextSampleTime) - hrtime2ns(this[kLastSampleTime])

    if (this[kSamplerOptions].collect.eventLoopDelay) {
      this[kEventLoopDelay][kSample](elapsedNs, this[kSamplerOptions].sampleInterval)
    }

    if (this[kSamplerOptions].collect.activeHandles) {
      this[kActiveHandles] = process._getActiveHandles().length
    }

    if (this[kSamplerOptions].collect.cpu) {
      this[kCpu][kSample](elapsedNs)
    }

    if (this[kSamplerOptions].collect.memory) {
      this[kMemory] = process.memoryUsage()
    }

    this[kLastSampleTime] = nextSampleTime
  }

  [kReset] () {
    if (this[kSamplerOptions].collect.eventLoopDelay) {
      this[kEventLoopDelay][kReset]()
    }

    if (this[kSamplerOptions].collect.cpu) {
      this[kCpu][kReset]()
    }

    if (this[kSamplerOptions].collect.gc) {
      this[kGC][kReset]()
    }
  }
}

module.exports = Sampler
